import { AutoLoginData } from "../dtos/patient.dto";
...
export class Apis {
  ...
  setAutoLoginData(autoLoginData: AutoLoginData) {
    localStorage.setItem('AUTO_LOGIN_DATA', JSON.stringify(autoLoginData));
  }

  getAutoLoginData(): AutoLoginData {
    const data = localStorage.getItem('AUTO_LOGIN_DATA');
    if (data) {
      return JSON.parse(data);
    }
    return new AutoLoginData();
  }

  clearAutoLoginData(): any {
    return localStorage.removeItem('AUTO_LOGIN_DATA');
  }
